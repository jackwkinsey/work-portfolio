...
def lambda_handler(event, context):
    ...
    topic = sns.Topic('arn:aws:sns:us-east-2:144055463112:deploy_portfolio_topic')
    ...
    try:
        ...
        with zipfile.ZipFile(build_zip) as myzip:
            for name in myzip.namelist():
                ...
                portfolio_bucket.upload_fileobj(
                    obj, name, ExtraArgs={'ContentType': mimetype})
                portfolio_bucket.Object(name).Acl().put(ACL='public-read')
        print('FINISHED!')
        
        topic.publish(Subject='[SUCCESS] Portfolio Deployed', Message='Portfolio deployed successfully!')
        ...
        topic.publish(Subject='[FAIL] Portfolio Not Deployed', Message='Portfolio was not deployed.')
        raise